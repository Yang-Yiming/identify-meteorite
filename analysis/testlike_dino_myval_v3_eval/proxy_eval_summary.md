...
| run | known_test_f1 | known_myval_f1_from_docs | f1_at_0_5__myval_masked | f1_at_0_5__testlike_cluster_dino_v3 | f1_at_0_5__testlike_top_dino_v3 | best_f1__testlike_cluster_dino_v3 | note |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| testlike_cluster_dino_v3 | f1_at_0_5 | -0.6377 |
| testlike_cluster_dino_v3 | best_f1 | -0.4638 |
| testlike_cluster_dino_v3 | prob_mean | 0.4638 |
| testlike_top_dino_v3 | f1_at_0_5 | -0.5885 |
| testlike_top_dino_v3 | best_f1 | -0.5798 |
| testlike_top_dino_v3 | prob_mean | 0.6377 |
...
DINO embeddings improve the absolute realism of the proxy relative to the
handcrafted baseline, but they still do not solve the ranking problem. The
proxy remains too friendly to the known bad mytest-heavy runs, and it still
cannot distinguish different post-process policies on the same checkpoint.

This is still useful: it confirms that a frozen third-party embedding is a
better space to work in than a trained checkpoint embedding, but the selection
rule probably needs additional target-aware logic or manual audit of the
candidate set.
