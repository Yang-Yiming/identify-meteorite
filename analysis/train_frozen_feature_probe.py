#!/usr/bin/env python3
"""Train simple frozen-feature probes and evaluate directly on Testlike V4."""
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("analysis/frozen_probe_siglip"))
    ...
    if args.feature_cache is None:
        ...
        args.feature_cache = args.out_dir / f"{safe_name}_features.npz"
    ...
    if args.feature_cache.is_file():
        features = np.load(args.feature_cache)["features"]
    else:
    ...
    for c in c_values:
        ...
        row = {"model_name": args.model_name, "C": c, "class_weight": args.class_weight}
        for item in evals:
        ...
        full_labels = np.array([pred_by_id.get(tid, int(baseline_labels[tid])) for tid in full_ids], dtype=int)
        base_labels = np.array([int(baseline_labels[tid]) for tid in full_ids], dtype=int)
        missing_test_ids = [tid for tid in full_ids if tid not in pred_by_id]
        row["test_available_n"] = int(len(test_ids))
        row["test_missing_filled_from_baseline"] = int(len(missing_test_ids))
        row["test_available_pos_pred_at_0_5"] = int(preds.sum())
        row["test_pos_pred_at_0_5"] = int(full_labels.sum())
        row["test_prob_mean"] = float(test_probs.mean())
        row["diff_vs_baseline"] = int((full_labels != base_labels).sum())
        row["baseline_pos_to_neg"] = int(((base_labels == 1) & (full_labels == 0)).sum())
        row["baseline_neg_to_pos"] = int(((base_labels == 0) & (full_labels == 1)).sum())
        rows.append(row)
        ...
        sub_path = args.out_dir / f"submission_C{str(c).replace('.', 'p')}.csv"
        sub.to_csv(sub_path, index=False)
        sub_probs.assign(C=c).to_csv(args.out_dir / f"submission_probs_C{str(c).replace('.', 'p')}.csv", index=False)
        submissions.append(str(sub_path))

    results = pd.DataFrame(rows).sort_values(
        ["v4_cluster_f1_at_0_5", "v4_top_f1_at_0_5", "diff_vs_baseline"],
    ...
    report_cols = [
        ...
        "v4_top_f1_at_0_5",
        "myval_f1_at_0_5",
        "test_available_pos_pred_at_0_5",
        "test_pos_pred_at_0_5",
        "test_missing_filled_from_baseline",
        "diff_vs_baseline",
        "baseline_pos_to_neg",
        "baseline_neg_to_pos",
    ]
    report = [
        ...
        "",
        f"Backbone: {args.model_name}",
        f"Class weight: {args.class_weight}",
        "",
        results[report_cols].to_csv(index=False),
    ]
    ...
    print(results[report_cols].to_string(index=False))
