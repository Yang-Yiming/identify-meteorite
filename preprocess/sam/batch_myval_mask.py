#!/usr/bin/env python3
"""
Batch process myval images with SAM3 text prompt "stone".
Saves binary masks to output/myval.
...
DEFAULT_INPUT_DIR = os.path.join(SCRIPT_DIR, "data", "myval", "images")
DEFAULT_OUTPUT_DIR = os.path.join(SCRIPT_DIR, "output", "myval")
...
def main():
    parser = argparse.ArgumentParser(description="SAM3 batch myval stone mask inference")
    ...
    n, skipped = process_directory(
        processor, input_dir, output_dir, resume=resume
    )
    if skipped:
